    ...
    document.addEventListener("click", function (ea) {
        ...
        if (ea.target.tagName == 'TH' && ea.target.dataset.sortable=="true") {
            ...
            if (document.querySelector(".mdl-layout__tab-panel.is-active") == tab1) {
                ...
                mydevices.getCurrentDecice(userId,searchbox.value, new Sort(token).getSortingUrl(ea.target));
            }
            else if(document.querySelector(".mdl-layout__tab-panel.is-active") == tab2) {

                mydevices.getPreviousDecice(userId,searchbox.value, new Sort(token).getSortingUrl(ea.target));
            }
            else{
                mydevices.getRequestHistory(userId,searchbox.value,new Sort(token).getSortingUrl(ea.target));
            }
        }
