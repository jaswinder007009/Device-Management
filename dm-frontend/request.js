"use strict";
var adminId = 16;
var searchBar = "<br><input type='text' class='mdl-input-field' placeholder='Enter text to Search' id='searchBar' style='margin-left:1000px'>"
    + "</input><button class='search-button'>Search</button><br>";
var headingArray = ['User Id', 'Model', 'Brand', 'Type', 'RAM', 'Storage',
...
document.addEventListener("click", function (e) {
    if (e.target.className == "reject-button") {
        var data = e.target.dataset.requestid;
        var requestId = parseInt(data, 10);
        requestAction(requestId + '/reject?id=' + adminId);
        alert("Request " + requestId + " rejected");
        document.getElementById("content").innerHTML = "";
        getPendingRequests("http://localhost:5000/api/request/pending");
    }
    if (e.target.className == "accept-button") {
        var data = e.target.dataset.requestid;
        var requestId = parseInt(data, 10);
