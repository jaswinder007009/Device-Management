...
public int whatIs(String jass1)
{
 return jass1=="inactive"?1:2;
         ...
         public void MarkUserInactive(int jass)
          {
              using var cmd = Db.Connection.CreateCommand();
              cmd.CommandText = @"UPDATE `user` SET `status_id`="+ jass +" WHERE `user_id` = @userr_id;";
              Binduser_id(cmd);
              cmd.ExecuteNonQuery();
          }
        ...
        internal string GetAllUsersquery = @"select user_id,salutation,first_name,middle_name,last_name,role_name,department_name,designation as 'designation_name',email,gender,date_of_birth,date_of_joining,status.status_name,
